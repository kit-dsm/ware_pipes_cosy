...
def load_df_results(path: Path) -> pd.DataFrame:
    ...
    df = pd.read_pickle(path) if path.suffix == ".pkl" else read_results(path)
...
def build_dashboard(df: pd.DataFrame, site_data_dir: Path) -> None:
    raw = prepare_raw(df)
    ...
    print(f"Wrote dashboard data to: {site_data_dir}")
